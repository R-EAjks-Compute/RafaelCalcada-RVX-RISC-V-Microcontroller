...
module rvx_core #(
  ...
  assign slli = op_imm_type & instruction_funct3 == `RISCV_FUNCT3_SLLI & 
    instruction_funct7 == `RISCV_FUNCT7_SLLI;

  assign srli = op_imm_type & instruction_funct3 ==
      `RISCV_FUNCT3_SRLI & instruction_funct7 == `RISCV_FUNCT7_SRLI;

  assign srai = op_imm_type & instruction_funct3 == `RISCV_FUNCT3_SRAI &
      instruction_funct7 == `RISCV_FUNCT7_SRAI;

  assign add = op_type & instruction_funct3 ==
      `RISCV_FUNCT3_ADD
      & instruction_funct7 == `RISCV_FUNCT7_ADD;

  assign sub = op_type & instruction_funct3 ==
      `RISCV_FUNCT3_SUB
      & instruction_funct7 == `RISCV_FUNCT7_SUB;

  assign slt = op_type & instruction_funct3 ==
      `RISCV_FUNCT3_SLT
      & instruction_funct7 == `RISCV_FUNCT7_SLT;

  assign sltu = op_type & instruction_funct3 ==
      `RISCV_FUNCT3_SLTU
      & instruction_funct7 == `RISCV_FUNCT7_SLTU;

  assign is_and = op_type & instruction_funct3 ==
      `RISCV_FUNCT3_AND
      & instruction_funct7 == `RISCV_FUNCT7_AND;

  assign is_or = op_type & instruction_funct3 ==
      `RISCV_FUNCT3_OR
      & instruction_funct7 == `RISCV_FUNCT7_OR;

  assign is_xor = op_type & instruction_funct3 ==
      `RISCV_FUNCT3_XOR
      & instruction_funct7 == `RISCV_FUNCT7_XOR;

  assign sll = op_type & instruction_funct3 ==
      `RISCV_FUNCT3_SLL
      & instruction_funct7 == `RISCV_FUNCT7_SLL;

  assign srl = op_type & instruction_funct3 ==
      `RISCV_FUNCT3_SRL
      & instruction_funct7 == `RISCV_FUNCT7_SRL;

  assign sra = op_type & instruction_funct3 ==
      `RISCV_FUNCT3_SRA
      & instruction_funct7 == `RISCV_FUNCT7_SRA;

  assign csrxxx = system_type & instruction_funct3 != 3'b000 & instruction_funct3 != 3'b100;

  assign ecall =
    system_type &
    instruction_funct3 == `RISCV_FUNCT3_ECALL &
    instruction_funct7 == `RISCV_FUNCT7_ECALL &
    instruction_rs1_address == `RISCV_RS1_ECALL &
    instruction_rs2_address == `RISCV_RS2_ECALL &
    instruction_rd_address  == `RISCV_RD_ECALL;

  assign ebreak =
    system_type &
    instruction_funct3 == `RISCV_FUNCT3_EBREAK &
    instruction_funct7 == `RISCV_FUNCT7_EBREAK &
    instruction_rs1_address == `RISCV_RS1_EBREAK &
    instruction_rs2_address == `RISCV_RS2_EBREAK &
    instruction_rd_address  == `RISCV_RD_EBREAK;

  assign mret =
    system_type &
    instruction_funct3 == `RISCV_FUNCT3_MRET &
    instruction_funct7 == `RISCV_FUNCT7_MRET &
    instruction_rs1_address == `RISCV_RS1_MRET &
    instruction_rs2_address == `RISCV_RS2_MRET &
  ...
  always @* begin : immediate_type_decoding
    if (op_imm_type == 1'b1 || load_type == 1'b1 || jalr_type == 1'b1)
      immediate_type = `RISCV_I_TYPE_IMMEDIATE;
  ...
  assign immediate_b_type = {
    immediate_sign_extension, instruction[7], instruction[30:25], instruction[11:8], 1'b0
  };
  ...
  assign flush = current_state != `RVX_MMODE_STATE_OPERATING;
  ...
  always @* begin : m_mode_fsm_next_state_logic
    case (current_state)
      `RVX_MMODE_STATE_RESET: next_state = `RVX_MMODE_STATE_OPERATING;
      `RVX_MMODE_STATE_OPERATING:
      if (take_trap) next_state = `RVX_MMODE_STATE_TRAP_TAKEN;
      else if (mret) next_state = `RVX_MMODE_STATE_TRAP_RETURN;
      else next_state = `RVX_MMODE_STATE_OPERATING;
      `RVX_MMODE_STATE_TRAP_TAKEN: next_state = `RVX_MMODE_STATE_OPERATING;
      `RVX_MMODE_STATE_TRAP_RETURN: next_state = `RVX_MMODE_STATE_OPERATING;
      default: next_state = `RVX_MMODE_STATE_OPERATING;
    endcase
  end

  always @(posedge clock) begin : m_mode_fsm_current_state_register
    if (reset_internal) current_state <= `RVX_MMODE_STATE_RESET;
    else if (clock_enable | interrupt_pending) current_state <= next_state;
  end

  always @* begin : program_counter_source_mux
    case (current_state)
      `RVX_MMODE_STATE_RESET: program_counter_source = `RVX_PC_BOOT;
      `RVX_MMODE_STATE_OPERATING: program_counter_source = `RVX_PC_NEXT;
      `RVX_MMODE_STATE_TRAP_TAKEN: program_counter_source = `RVX_PC_TRAP;
      `RVX_MMODE_STATE_TRAP_RETURN: program_counter_source = `RVX_PC_EPC;
      default: program_counter_source = `RVX_PC_NEXT;
    endcase
  end

  assign irq_external_response =
    (current_state    == `RVX_MMODE_STATE_TRAP_TAKEN) &&
    (csr_mcause_code  == 5'b1011);

  assign irq_timer_response = (current_state == `RVX_MMODE_STATE_TRAP_TAKEN) && (csr_mcause_code == 5'b0111);
  ...
  always @(posedge clock) begin : mstatus_csr_fields_update
    if (reset_internal) begin
      ...
      if (current_state == `RVX_MMODE_STATE_TRAP_RETURN) begin
        csr_mstatus_mie  <= csr_mstatus_mpie;
        csr_mstatus_mpie <= 1'b1;
      end else if (current_state == `RVX_MMODE_STATE_TRAP_TAKEN) begin
        csr_mstatus_mpie <= csr_mstatus_mie;
        csr_mstatus_mie  <= 1'b0;
      end
      else if(current_state == `RVX_MMODE_STATE_OPERATING && instruction_csr_address == `RISCV_CSR_MSTATUS_ADDR && csr_file_write_enable) begin
  ...
  always @(posedge clock) begin : mie_csr_fields_implementation
    if (reset_internal) begin
      ...
      csr_mie_mtie <= 1'b0;
      csr_mie_msie <= 1'b0;
    end else if (clock_enable & instruction_csr_address == `RISCV_CSR_MIE_ADDR && csr_file_write_enable) begin
  ...
  always @(posedge clock) begin : mepc_implementation
    if (reset_internal) csr_mepc <= 32'h00000000;
      ...
      else if(current_state == `RVX_MMODE_STATE_OPERATING && instruction_csr_address == `RISCV_CSR_MEPC_ADDR && csr_file_write_enable)
  ...
  always @(posedge clock) begin : minstret_implementation
    if (reset_internal) csr_minstret <= 64'b0;
      ...
      if (instruction_csr_address == `RISCV_CSR_MINSTRET_ADDR && csr_file_write_enable) begin
        if (current_state == `RVX_MMODE_STATE_OPERATING)
          csr_minstret <= {csr_minstret[63:32], csr_write_data} + 1;
        else csr_minstret <= {csr_minstret[63:32], csr_write_data};
      end else if (instruction_csr_address == `RISCV_CSR_MINSTRETH_ADDR && csr_file_write_enable) begin
        if (current_state == `RVX_MMODE_STATE_OPERATING)
          csr_minstret <= {csr_write_data, csr_minstret[31:0]} + 1;
        else csr_minstret <= {csr_write_data, csr_minstret[31:0]};
      end else begin
        if (current_state == `RVX_MMODE_STATE_OPERATING) csr_minstret <= csr_minstret + 1;
  ...
  always @(posedge clock) begin : mcause_implementation
    if (reset_internal) csr_mcause <= 32'h00000000;
      ...
      if (current_state == `RVX_MMODE_STATE_TRAP_TAKEN)
        csr_mcause <= {csr_mcause_interrupt_flag, 26'b0, csr_mcause_code};
      else if(current_state == `RVX_MMODE_STATE_OPERATING && instruction_csr_address == `RISCV_CSR_MCAUSE_ADDR && csr_file_write_enable)
  ...
  always @(posedge clock) begin : trap_cause_implementation
    ...
    if (clock_enable & current_state == `RVX_MMODE_STATE_OPERATING) begin
      if (illegal_instruction) begin
        csr_mcause_code           <= 5'b0010;
        csr_mcause_interrupt_flag <= 1'b0;
      end else if (misaligned_instruction_address) begin
        ...
        csr_mcause_interrupt_flag <= 1'b0;
      end else if (csr_mstatus_mie & csr_mie_mfie[0] & csr_mip_mfip[0]) begin
        csr_mcause_code           <= 5'd16;
        csr_mcause_interrupt_flag <= 1'b1;
      end else if (csr_mstatus_mie & csr_mie_mfie[1] & csr_mip_mfip[1]) begin
        csr_mcause_code           <= 5'd17;
        csr_mcause_interrupt_flag <= 1'b1;
      end else if (csr_mstatus_mie & csr_mie_mfie[2] & csr_mip_mfip[2]) begin
        csr_mcause_code           <= 5'd18;
        csr_mcause_interrupt_flag <= 1'b1;
      end else if (csr_mstatus_mie & csr_mie_mfie[3] & csr_mip_mfip[3]) begin
        csr_mcause_code           <= 5'd19;
        csr_mcause_interrupt_flag <= 1'b1;
      end else if (csr_mstatus_mie & csr_mie_mfie[4] & csr_mip_mfip[4]) begin
        csr_mcause_code           <= 5'd20;
        csr_mcause_interrupt_flag <= 1'b1;
      end else if (csr_mstatus_mie & csr_mie_mfie[5] & csr_mip_mfip[5]) begin
        csr_mcause_code           <= 5'd21;
        csr_mcause_interrupt_flag <= 1'b1;
      end else if (csr_mstatus_mie & csr_mie_mfie[6] & csr_mip_mfip[6]) begin
        csr_mcause_code           <= 5'd22;
        csr_mcause_interrupt_flag <= 1'b1;
      end else if (csr_mstatus_mie & csr_mie_mfie[7] & csr_mip_mfip[7]) begin
        csr_mcause_code           <= 5'd23;
        csr_mcause_interrupt_flag <= 1'b1;
      end else if (csr_mstatus_mie & csr_mie_mfie[8] & csr_mip_mfip[8]) begin
        csr_mcause_code           <= 5'd24;
        csr_mcause_interrupt_flag <= 1'b1;
      end else if (csr_mstatus_mie & csr_mie_mfie[9] & csr_mip_mfip[9]) begin
        csr_mcause_code           <= 5'd25;
        csr_mcause_interrupt_flag <= 1'b1;
      end else if (csr_mstatus_mie & csr_mie_mfie[10] & csr_mip_mfip[10]) begin
        csr_mcause_code           <= 5'd26;
        csr_mcause_interrupt_flag <= 1'b1;
      end else if (csr_mstatus_mie & csr_mie_mfie[11] & csr_mip_mfip[11]) begin
        csr_mcause_code           <= 5'd27;
        csr_mcause_interrupt_flag <= 1'b1;
      end else if (csr_mstatus_mie & csr_mie_mfie[12] & csr_mip_mfip[12]) begin
        csr_mcause_code           <= 5'd28;
        csr_mcause_interrupt_flag <= 1'b1;
      end else if (csr_mstatus_mie & csr_mie_mfie[13] & csr_mip_mfip[13]) begin
        csr_mcause_code           <= 5'd29;
        csr_mcause_interrupt_flag <= 1'b1;
      end else if (csr_mstatus_mie & csr_mie_mfie[14] & csr_mip_mfip[14]) begin
        csr_mcause_code           <= 5'd30;
        csr_mcause_interrupt_flag <= 1'b1;
      end else if (csr_mstatus_mie & csr_mie_mfie[15] & csr_mip_mfip[15]) begin
        csr_mcause_code           <= 5'd31;
        csr_mcause_interrupt_flag <= 1'b1;
      end else if (csr_mstatus_mie & csr_mie_meie & csr_mip_meip) begin
        csr_mcause_code           <= 5'b1011;
        csr_mcause_interrupt_flag <= 1'b1;
  ...
  always @(posedge clock) begin : mtval_implementation
    if (reset_internal) csr_mtval <= 32'h00000000;
      ...
      else if(current_state == `RVX_MMODE_STATE_OPERATING && instruction_csr_address == `RISCV_CSR_MTVAL_ADDR && csr_file_write_enable)
