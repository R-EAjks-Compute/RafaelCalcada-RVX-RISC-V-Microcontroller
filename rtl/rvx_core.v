...
module rvx_core #(
  ...
  always @(posedge clock) begin : mstatus_csr_fields_update
    if (reset_internal) begin
      ...
      if (current_state == STATE_TRAP_RETURN) begin
        ...
        csr_mstatus_mpie <= 1'b1;
      end else if (current_state == STATE_TRAP_TAKEN) begin
  ...
  always @(posedge clock) begin : mie_csr_fields_implementation
    if (reset_internal) begin
      ...
      csr_mie_mtie <= 1'b0;
      csr_mie_msie <= 1'b0;
    end else if (clock_enable & instruction_csr_address == MIE && csr_file_write_enable) begin
  ...
  always @(posedge clock) begin : minstret_implementation
    if (reset_internal) csr_minstret <= 64'b0;
      ...
      end else if (instruction_csr_address == MINSTRETH && csr_file_write_enable) begin
        if (current_state == STATE_OPERATING)
